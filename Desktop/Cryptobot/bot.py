...
import aiohttp  # у тебя flask и прочее уже есть, но на всякий добавляем
...
# URL твоего n8n-воркфлоу, который на выходе отдаёт { "text": "готовый пост" }
N8N_POST_URL = "https://makarius2018.app.n8n.cloud/webhook/daily-post"  # ← свою ссылку поставь
...
async def autopublish_job(bot):
    """
    Ежедневный автопостинг: забираем готовый текст из n8n и шлём в канал.
    """
    chat_id = CHANNEL_USERNAME  # у тебя это уже задано наверху

    # 1. тянем пост из n8n
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(N8N_POST_URL, timeout=20) as resp:
                data = await resp.json()
    except Exception as e:
        logging.error(f"[autopublish_job] не удалось получить пост из n8n: {e}")
        return

    text = data.get("text") or data.get("message") or data.get("content")
    if not text:
        logging.error("[autopublish_job] n8n вернул пустой текст")
        return
    ...
    try:
        ...
        logging.info("[autopublish_job] пост опубликован в канал")
    except Exception as e:
        logging.error(f"[autopublish_job] ошибка отправки в канал: {e}")
